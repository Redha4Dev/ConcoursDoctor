...
export default function ProgramDetailPage() {
  ...
  if (error || !program) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen bg-[#F8F9FA] gap-4">
        ...
        <button onClick={() => router.push("/dashboard/coordinator/programs")} className="text-[#3014B8] underline">
          Return to Programs
        </button>
      </div>
    );
  ...
  return (
    ...
    >
      {/* Breadcrumb */}
      <button
        onClick={() => router.push("/dashboard/coordinator/programs")}
              ...
              {program.sessions && program.sessions.length > 0 ? (
                program.sessions.map((session) => (
                  <div
                    ...
                    style={{ height: 83 }}
                    onClick={() => router.push(`/dashboard/coordinator/programs/${program.id}/${session.id}`)}
                  >
