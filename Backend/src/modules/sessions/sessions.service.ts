...
const getSessionOrThrow = async (id: string) => {
  const session = await identityDb.competitionSession.findUnique({ where: { id } })
  if (!session) throw new AppError('Session not found', 404)
  return session
}

const assertDraft = (status: string) => {
  if (status !== 'DRAFT') throw new AppError('Session can only be modified while in DRAFT status', 403)
}
...
export const updateSubject = async (sessionId: string, subjectId: string, dto: UpdateSubjectDto) => {
  const session = await getSessionOrThrow(sessionId)
  assertDraft(session.status)
...
export const getGradingConfig = async (sessionId: string) => {
  ...
  const config = await identityDb.gradingConfig.findUnique({ where: { sessionId } })
...
export const setGradingConfig = async (
  ...
  return identityDb.gradingConfig.upsert({
    where:  { sessionId },
    update: { discrepancyThreshold: dto.discrepancyThreshold, configuredBy, configuredAt: new Date() },
    create: { sessionId, discrepancyThreshold: dto.discrepancyThreshold, configuredBy },
  })
}